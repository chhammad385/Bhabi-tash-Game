...
import { User, LogIn, UserPlus, LogOut, Check, Sparkles, KeyRound, AtSign, ShieldAlert, Volume2, VolumeX } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const [soundsMuted, setSoundsMuted] = useState(sounds.getIsMuted());
          ...
          <div className="flex flex-col gap-5 max-h-[65vh] overflow-y-auto pr-1 scrollbar-thin">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-bold text-white uppercase tracking-wide">Game Sounds</h4>
              </div>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  // Play a confirmation tone when switching sound back on.
                  if (!next) sounds.playCardPlay();
                }}
                className="w-full flex items-center justify-between gap-3 p-3 rounded-xl bg-slate-800/40 border border-slate-700/60 hover:border-slate-600 transition text-left"
              >
                <span className="min-w-0">
                  <span className="block text-xs font-semibold text-slate-200">
                    Sound effects {soundsMuted ? 'off' : 'on'}
                  </span>
                  <span className="block text-[10px] text-slate-500 leading-relaxed mt-0.5">
                    Card deal and play sounds, Tochoo alert, your-turn chime, the Bhabhi fanfare and
                    chat pops. Does not affect anything else.
                  </span>
                </span>

                <span
                  className={`relative shrink-0 w-10 h-5 rounded-full transition-colors ${
                    soundsMuted ? 'bg-slate-700' : 'bg-emerald-600'
                  }`}
                >
                  <span
                    className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${
                      soundsMuted ? 'left-0.5' : 'left-[22px]'
                    }`}
                  />
                </span>
              </button>
